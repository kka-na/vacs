...
const scenarioTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/scenario",
  messageType: "geometry_msgs/Vector3",
});
const accTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/acc_state",
  messageType: "std_msgs/Bool",
});
const epsTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/eps_state",
  messageType: "std_msgs/Bool",
});
const ignoreTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/ignore_state",
  messageType: "std_msgs/Bool",
});
const blankTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/blank_state",
  messageType: "std_msgs/Bool",
});
...
const stopAtTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/stop_at_state",
  messageType: "std_msgs/Bool",
});
const keepLaneTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/keep_lane_state",
  messageType: "std_msgs/Bool",
});
const prepLeftTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/prep_left_state",
  messageType: "std_msgs/Bool",
});
const prepRightTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/prep_right_state",
  messageType: "std_msgs/Bool",
});
const lcLeftTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/lc_left_state",
  messageType: "std_msgs/Bool",
});
const lcRightTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/lc_right_state",
  messageType: "std_msgs/Bool",
});
const readyTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/ready_state",
  messageType: "std_msgs/Bool",
});
const gPlanTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/g_plan_state",
  messageType: "std_msgs/Bool",
});
const bPlanTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/b_plan_state",
  messageType: "std_msgs/Bool",
});
const vPlanTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/v_plan_state",
  messageType: "std_msgs/Bool",
});
const wPlanTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/w_plan_state",
  messageType: "std_msgs/Bool",
});
const bothPlanTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/both_plan_state",
  messageType: "std_msgs/Bool",
});
const emptyTopic = new ROSLIB.Topic({
  ros: ros,
  name: "/empty_state",
  messageType: "std_msgs/Bool",
});
...
const SetState = (props) => {
  ...
  const controlsChange = (event, control) => {
    setControls(control);
    if (isSub) {
      if (!acc && control.includes("acc")) {
        accTopic.publish({ data: true });
        setACC(true);
      }
      if (!eps && control.includes("eps")) {
        epsTopic.publish({ data: true });
        setEps(true);
      }
      if (!ignore && control.includes("ignore")) {
        ignoreTopic.publish({ data: true });
        setIgnore(true);
      }
      if (!blank && control.includes("blank")) {
        blankTopic.publish({ data: true });
        setBlank(true);
      }
    }
    if (isSub) {
      if (acc && !control.includes("acc")) {
        accTopic.publish({ data: false });
        setACC(false);
      }
      if (eps && !control.includes("eps")) {
        epsTopic.publish({ data: false });
        setEps(false);
      }
      if (ignore && !control.includes("ignore")) {
        ignoreTopic.publish({ data: false });
        setIgnore(false);
      }
      if (blank && !control.includes("blank")) {
        blankTopic.publish({ data: false });
        setBlank(false);
      }
    }
  };
  const plansChange = (event, plan) => {
    setPlans(plan);
    if (isSub) {
      if (!stop && plan.includes("stop")) {
        stopTopic.publish({ data: true });
        setStop(true);
      }
      if (!stopAt && plan.includes("stopAt")) {
        stopAtTopic.publish({ data: true });
        setStopAt(true);
      }
      if (!keepLane && plan.includes("keepLane")) {
        keepLaneTopic.publish({ data: true });
        setKeepLane(true);
      }
      if (!prepLeft && plan.includes("prepLeft")) {
        prepLeftTopic.publish({ data: true });
        setPrepLeft(true);
      }
      if (!prepRight && plan.includes("prepRight")) {
        prepRightTopic.publish({ data: true });
        setPrepRight(true);
      }
      if (!lcLeft && plan.includes("lcLeft")) {
        lcLeftTopic.publish({ data: true });
        setLcLeft(true);
      }
      if (!lcRight && plan.includes("lcRight")) {
        lcRightTopic.publish({ data: true });
        setLcRight(true);
      }
    }
    if (isSub) {
      if (stop && !plan.includes("stop")) {
        stopTopic.publish({ data: false });
        setStop(false);
      }
      if (stopAt && !plan.includes("stopAt")) {
        stopAtTopic.publish({ data: false });
        setStopAt(false);
      }
      if (keepLane && !plan.includes("keepLane")) {
        keepLaneTopic.publish({ data: false });
        setKeepLane(false);
      }
      if (prepLeft && !plan.includes("prepLeft")) {
        prepLeftTopic.publish({ data: false });
        setPrepLeft(false);
      }
      if (prepRight && !plan.includes("prepRight")) {
        prepRightTopic.publish({ data: false });
        setPrepRight(false);
      }
      if (lcLeft && !plan.includes("lcLeft")) {
        lcLeftTopic.publish({ data: false });
        setLcLeft(false);
      }
      if (lcRight && !plan.includes("lcRight")) {
        lcRightTopic.publish({ data: false });
        setLcRight(false);
      }
    }
  };
  ...
  return (
    <Grid item xs container direction="column" spacing={1}>
      ...
      <ToggleSets
        exclusive={true}
        toggleValues={scenarios}
        toggleChange={scenariosChange}
      ...
      <ToggleSets
        exclusive={false}
        toggleValues={controls}
        toggleChange={controlsChange}
